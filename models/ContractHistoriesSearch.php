<?php

namespace app\models;

use yii\base\Model;
use yii\data\ActiveDataProvider;
use app\models\ContractHistories;

/**
 * ContractHistoriesSearch represents the model behind the search form of `app\models\ContractHistories`.
 */
class ContractHistoriesSearch extends ContractHistories
{
    /**
     * {@inheritdoc}
     */
    public function rules()
    {
        return [
            [['id', 'duration_contract', 'id_employee'], 'integer'],
            [['start_contract', 'number_contract', 'doh'], 'safe'],
            [['basic_salary'], 'number'],
            [['set_default'], 'boolean'],
        ];
    }

    /**
     * {@inheritdoc}
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = ContractHistories::find();

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
        $query->andFilterWhere([
            'id' => $this->id,
            'start_contract' => $this->start_contract,
            'duration_contract' => $this->duration_contract,
            'id_employee' => $this->id_employee,
            'doh' => $this->doh,
            'basic_salary' => $this->basic_salary,
            'set_default' => $this->set_default,
        ]);

        $query->andFilterWhere(['ilike', 'number_contract', $this->number_contract]);

        return $dataProvider;
    }
}
